...
const Notes: React.FC = () => {
  ...
  const [showCharLimitWarning, setShowCharLimitWarning] =
    ...
    try {
      ...
      if (cloudNotes) {
        ...
        const cloudNoteIds = new Set(
          cloudNotesFormatted.map((note) => note.id)
        );

        // Find notes to delete (in cloud but not in local)
        const notesToDelete = Array.from(cloudNoteIds).filter(
          (id) => !localNoteIds.has(id)
        );

        // Find notes to add (in local but not in cloud)
        const notesToAdd = Array.from(localNoteIds).filter(
          (id) => !cloudNoteIds.has(id)
        );

        // Find notes that exist in both local and cloud
        const commonNoteIds = Array.from(localNoteIds).filter((id) =>
          cloudNoteIds.has(id)
        );
        ...
        const notesToSync = sortedNotes
          .map((note) => {
            ...
            if (
              !cloudNote ||
              note.updatedAt > new Date(cloudNote.updatedAt).getTime()
            ) {
              return {
                id: note.id,
        ...
        <div className="flex items-center gap-2">
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent children={t("createNewNote")} />
            </Tooltip>
          </TooltipProvider>
        </div>
      </motion.div>
                ...
                <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent children={t("deleteNote")} />
                    </Tooltip>
                  </TooltipProvider>
                </div>
              </motion.div>
                  ...
                  <AnimatePresence>
                    {showCharLimitWarning && (
                      ...
                      >
                        ...
                        <span>
                          {t("characterLimitReached")} ({MAX_CONTENT_LENGTH})
                        </span>
                      </motion.div>
                    )}
                  </AnimatePresence>
