...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile, error: profileError } = await supabaseClient.from('profiles').select('polar_customer_id').eq('id', user.id).single();
    if (profileError) {
      throw profileError;
    }
    ...
    const response = await fetch(`${POLAR_API_URL_PROD}/customer-sessions`, {
      method: 'POST',
      headers: {
