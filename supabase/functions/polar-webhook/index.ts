...
async function verifyPolarSignature(request, body) {
  try {
    ...
    validateEvent(body, headers, Deno.env.get("POLAR_WEBHOOK_SECRET_PROD"));
    return true;
  } catch (error) {
...
// Utility functions
async function storeWebhookEvent(supabaseClient, body) {
  try {
    const { data, error } = await supabaseClient.from("webhook_events").insert({
...
// Event handlers
async function handleCheckoutCreated(supabaseClient, body) {
  try {
    ...
    return new Response(JSON.stringify({
      message: "handleCheckoutCreated created successfully"
    }), {
      status: 200,
      headers: {
        ...corsHeaders,
    ...
    return new Response(JSON.stringify({
      error: "Failed to handleCheckoutCreated"
    }), {
      status: 500,
      headers: {
        ...corsHeaders,
...
async function handleCheckoutUpdated(supabaseClient, body) {
  try {
    ...
    return new Response(JSON.stringify({
      message: "handleCheckoutUpdated created successfully"
    }), {
      status: 200,
      headers: {
        ...corsHeaders,
    ...
    return new Response(JSON.stringify({
      error: "Failed to create handleCheckoutUpdated"
    }), {
      status: 500,
      headers: {
        ...corsHeaders,
...
async function handleSubscriptionCreated(supabaseClient, body) {
  try {
    ...
    return new Response(JSON.stringify({
      message: "handleSubscriptionCreated created successfully"
    }), {
      status: 200,
      headers: {
        ...corsHeaders,
    ...
    return new Response(JSON.stringify({
      error: "Failed to handleSubscriptionCreated subscription"
    }), {
      status: 500,
      headers: {
        ...corsHeaders,
...
async function handleSubscriptionUpdated(supabaseClient, body) {
  try {
    const { data: existingSub } = await supabaseClient.from("subscriptions").select().eq("id", body.data.id).single();
    if (existingSub) {
    ...
    return new Response(JSON.stringify({
      message: "handleSubscriptionUpdated updated successfully"
    }), {
      status: 200,
      headers: {
        ...corsHeaders,
    ...
    return new Response(JSON.stringify({
      error: "Failed to handleSubscriptionUpdated subscription"
    }), {
      status: 500,
      headers: {
        ...corsHeaders,
...
async function handleSubscriptionRevoked(supabaseClient, body) {
  try {
    const { data: existingSub } = await supabaseClient.from("subscriptions").select().eq("id", body.data.id).single();
    if (existingSub) {
      const { error } = await supabaseClient.from("subscriptions").update({
    ...
    return new Response(JSON.stringify({
      message: "handleSubscriptionRevoked updated successfully"
    }), {
      status: 200,
      headers: {
        ...corsHeaders,
...
async function handleSubscriptionCanceled(supabaseClient, body) {
  try {
    const { data: existingSub } = await supabaseClient.from("subscriptions").select().eq("id", body.data.id).single();
    if (existingSub) {
      const { error } = await supabaseClient.from("subscriptions").update({
